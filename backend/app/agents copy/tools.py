...
def search_products_tool(
    ...
    If field is empty then provide empty string "" / 0 , did not pass null
    """
    db = SessionLocal()
    try:
...
- Otherwise → {{"type": "none", "target": null}}""",
        ...
        action:[
            {{"type":"enter", "target":{{element_selector}}, "data":"{{quantity}}", "require_confirmation": false}},
...
- For navigation, use {{"type": "navigate", "target": "/path", "data": null}}.
...
- For entering text, use {{"type": "enter", "target": "form_field_selector", "data": "text to enter", "require_confirmation": false}}.
