...
- If request needed action and type is 'clcik' then mention short note on "data":{{"Need permission to add the product to cart"}} or similar based on action."""
...
async def run_agent(message: str, session_id: str, conversation_history: list = None, mode: str = "text", page_type: Optional[str] = None, route: Optional[str] = None):
    ...
    try:
        ...
        if message == 'shared current page context':
            notPageContext = False
        # Add user message to session history
        if message and notPageContext:
